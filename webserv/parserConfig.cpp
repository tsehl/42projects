...
int check_values(std::vector<std::string> &tab_lines, std::vector<Server>& server, 
    std::vector<std::string> &tab_keys,  std::map<size_t, std::string> &map, std::map<std::string, std::string>& map_return)
{
    ...
    if (tab_lines[0].compare("return") == 0 && !is_double(tab_lines[0], tab_keys))
    {
        if (check_return(tab_lines[1], tab_lines[2]))
        {
            map_return.insert(std::make_pair(tab_lines[1], tab_lines[2]));
            server.back().setMapReturn(map_return);
            return (1);
        }
    }
...
int check_line(std::string line, std::vector<Server>& server, std::vector<std::string> &tab_keys, std::ifstream *configfile, std::vector<std::string> &tab_paths_location, 
    std::map<size_t, std::string> &map, std::vector<Location>& vec_location,  std::map<std::string, std::string>& map_location, std::map<std::string, std::string>& map_return)
{
    ...
    if (!check_values(tab_lines, server, tab_keys, map, map_return) && !is_closed(line))
        return (0);
    return (1);
}
...
int check_server(std::ifstream *configfile, std::vector<Server>& server)
{
    ...
    while (std::getline(*configfile, line))
    {
        ...
        if (is_server(line))
        {
            ...
            while (std::getline(*configfile, line) && !is_closed(line))
            {
                if(!check_line(line, server, tab_keys, configfile, tab_paths_location, map, vec_location, map_location, map_return))
                    return (0);    
            }
            if (!is_closed(line))
                return (0);
